# Build & run the NormalizeUrl test (ASCII only)
# Usage: powershell -File tests\run_normalizeurl_test.ps1
...
$exe = Join-Path $outDir "NormalizeUrlTest.exe"
$log = Join-Path $PSScriptRoot "normalizeurl_log.txt"
...
$src += (Join-Path $PSScriptRoot "NormalizeUrlTest.cs")

$cmdArgs = @("/nologo", "/target:exe", "/codepage:65001", "/main:LoongBrowser.NormalizeUrlTest", ("/out:" + $exe))
