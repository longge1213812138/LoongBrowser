# Build & run the history store unit test (ASCII only)
# Usage: powershell -File tests\run_history_test.ps1
...
$outDir = Join-Path $PSScriptRoot "_out_hist"
if (Test-Path $outDir) { Remove-Item $outDir -Recurse -Force }
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
Copy-Item (Join-Path $root "libs\*.dll") $outDir -Force

$exe = Join-Path $outDir "HistoryStoreTest.exe"
$log = Join-Path $PSScriptRoot "history_test_log.txt"
...
$src += (Join-Path $PSScriptRoot "HistoryStoreTest.cs")

$cmdArgs = @("/nologo", "/target:exe", "/codepage:65001", "/main:LoongBrowser.HistoryStoreTest", ("/out:" + $exe))
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Output "COMPILE FAILED - see tests\history_test_log.txt"
    exit 1
}
