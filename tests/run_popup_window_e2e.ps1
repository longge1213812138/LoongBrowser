# Build & run the popup window (always-on-top) end-to-end test (ASCII only)
# Usage: powershell -File tests\run_popup_window_e2e.ps1
...
$outDir = Join-Path $PSScriptRoot "_out_popupwin"
if (Test-Path $outDir) { Remove-Item $outDir -Recurse -Force }
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
Copy-Item (Join-Path $root "libs\*.dll") $outDir -Force

$exe = Join-Path $outDir "PopupWindowE2ETest.exe"
$log = Join-Path $PSScriptRoot "popup_window_e2e_log.txt"
...
$src += (Join-Path $PSScriptRoot "PopupWindowE2ETest.cs")

$cmdArgs = @("/nologo", "/target:exe", "/codepage:65001", "/main:LoongBrowser.PopupWindowE2ETest", ("/out:" + $exe))
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Output "COMPILE FAILED - see tests\popup_window_e2e_log.txt"
    exit 1
}
