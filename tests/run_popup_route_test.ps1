# Build & run the popup routing unit test (ASCII only)
# Usage: powershell -File tests\run_popup_route_test.ps1
...
$outDir = Join-Path $PSScriptRoot "_out_popup"
if (Test-Path $outDir) { Remove-Item $outDir -Recurse -Force }
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
Copy-Item (Join-Path $root "libs\*.dll") $outDir -Force

$exe = Join-Path $outDir "PopupRouteTest.exe"
$log = Join-Path $PSScriptRoot "popup_route_log.txt"
...
$src += (Join-Path $PSScriptRoot "PopupRouteTest.cs")

$cmdArgs = @("/nologo", "/target:exe", "/codepage:65001", "/main:LoongBrowser.PopupRouteTest", ("/out:" + $exe))
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Output "COMPILE FAILED - see tests\popup_route_log.txt"
    exit 1
}
