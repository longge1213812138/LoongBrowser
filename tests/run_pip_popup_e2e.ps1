# Build & run the PiP popup end-to-end test (ASCII only)
# Usage: powershell -File tests\run_pip_popup_e2e.ps1
...
$outDir = Join-Path $PSScriptRoot "_out_pip"
if (Test-Path $outDir) { Remove-Item $outDir -Recurse -Force }
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
Copy-Item (Join-Path $root "libs\*.dll") $outDir -Force

$exe = Join-Path $outDir "PipPopupE2ETest.exe"
$log = Join-Path $PSScriptRoot "pip_popup_e2e_log.txt"
...
$src += (Join-Path $PSScriptRoot "PipPopupE2ETest.cs")

$cmdArgs = @("/nologo", "/target:exe", "/codepage:65001", "/main:LoongBrowser.PipPopupE2ETest", ("/out:" + $exe))
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Output "COMPILE FAILED - see tests\pip_popup_e2e_log.txt"
    exit 1
}
