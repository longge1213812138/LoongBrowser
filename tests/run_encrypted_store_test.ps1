# Build & run the encrypted local-data regression test (ASCII only)
# Usage: powershell -File tests\run_encrypted_store_test.ps1
...
$outDir = Join-Path $PSScriptRoot "_out_enc"
if (Test-Path $outDir) { Remove-Item $outDir -Recurse -Force }
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
Copy-Item (Join-Path $root "libs\*.dll") $outDir -Force

$exe = Join-Path $outDir "EncryptedStoreTest.exe"
$log = Join-Path $PSScriptRoot "encrypted_store_test_log.txt"
...
$src += (Join-Path $PSScriptRoot "EncryptedStoreTest.cs")

$cmdArgs = @("/nologo", "/target:exe", "/codepage:65001", "/main:LoongBrowser.EncryptedStoreTest", ("/out:" + $exe))
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Output "COMPILE FAILED - see tests\encrypted_store_test_log.txt"
    exit 1
}
